...
def convertToCIE(image_name):
    ...
    cie = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # using cvtColor function to change the color space from RGB to CIE XYZ
    cie = cv2.cvtColor(cie, cv2.COLOR_RGB2XYZ) # using cvtColor function to change the color space from RGB to CIE XYZ
